...
#include "serialization.h"
    ...
    proto_tc::Stop Serialization::SaveStop(const Stop& stop) const {
        proto_tc::Stop result;
        result.set_name(stop.name);
        result.mutable_coords()->set_lat(stop.coord.lat);
        result.mutable_coords()->set_lon(stop.coord.lng);
        result.set_free(stop.free);
        return result;
    }

    proto_tc::FromToDistance Serialization::SaveFromToDistance(std::shared_ptr<Stop> from,
        std::shared_ptr<Stop> to, uint64_t distance) const {
    ...
    proto_tc::Bus Serialization::SaveBus(const Bus& bus) const {
        proto_tc::Bus result;
    ...
    void Serialization::LoadStop(const proto_tc::Stop& stop) {
        ...
        target.coord.lat = stop.coords().lat();
        target.coord.lng = stop.coords().lon();
    ...
    //    return base_.map();
    //}
    proto_tc::Map Serialization::LoadMap() const {
        return base_.map();
    }
